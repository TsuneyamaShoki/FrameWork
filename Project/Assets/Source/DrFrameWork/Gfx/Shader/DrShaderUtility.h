...
//! DrShaderUtility
namespace DrShaderUtility
{
	//! 
	enum class VARIABLE_TYPE : DrUint
	{
		FLOAT,
		INT,
		BOOL,
	...
	static const DrChar* VARIABLE_STRING[(DrUint)VARIABLE_TYPE::NUM] =
	{
		"Float",
		"Int",
		"Bool",
	...
	//! コンバート
	inline DrString ComvertShaderType(DR_SHADER_TYPE shaderType)
	{
		switch (shaderType)
		{
		case DR_SHADER_TYPE::VERTEX_SHADER:return "VetexShader";
		case DR_SHADER_TYPE::PIXEL_SHADER:return "PixlShader";
		case DR_SHADER_TYPE::GEOMETRY_SHADER:return "GiometoryShader";
		case DR_SHADER_TYPE::HULL_SHADER:return "HullShader";
		case DR_SHADER_TYPE::DOMAIN_SHADER:return "DomainShader";
		default:
		case DR_SHADER_TYPE::UNKNOWN_SHADER: return "UnknownShader";
		}
	}
	
	//! 判定 
	inline DrBool IsVS(DR_SHADER_TYPE type)
	{
		return ((DrUint)type & (DrUint)DR_SHADER_TYPE::VERTEX_SHADER) > 0;
	}
	inline DrBool IsPS(DR_SHADER_TYPE type)
	{
		return ((DrUint)type & (DrUint)DR_SHADER_TYPE::PIXEL_SHADER) > 0;
	}
	inline DrBool IsGS(DR_SHADER_TYPE type)
	{
		return ((DrUint)type & (DrUint)DR_SHADER_TYPE::GEOMETRY_SHADER) > 0;
	}
	inline DrBool IsHS(DR_SHADER_TYPE type)
	{
		return ((DrUint)type & (DrUint)DR_SHADER_TYPE::HULL_SHADER) > 0;
	}
	inline DrBool IsDS(DR_SHADER_TYPE type)
	{
		return ((DrUint)type & (DrUint)DR_SHADER_TYPE::DOMAIN_SHADER) > 0;
	}
	inline DrBool IsCS(DR_SHADER_TYPE type)
	{
		return ((DrUint)type & (DrUint)DR_SHADER_TYPE::COMPUTE_SHADER) > 0;
	}

	//! DXGI_FORMATへコンバート
	inline DXGI_FORMAT ComvertTextureFormat(const DrString& fmt)
	{
		if (fmt == "Unknown")
